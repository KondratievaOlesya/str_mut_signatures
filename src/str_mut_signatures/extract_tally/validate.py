...
# Required header fields for STR-annotated VCFs
REQUIRED_INFO_FIELDS = {"RU", "REF"}
REQUIRED_FORMAT_FIELDS = {"REPCN"}
...
def validate_vcf(vcf_path: str | Path) -> VCFValidationResult:
    ...
    - whether required STR annotations are defined in header:
        * INFO/RU
        * INFO/REF
        * FORMAT/REPCN
    ...
    -------
    VCFValidationResult
        has_str_annotations : True if all required INFO/FORMAT IDs exist.
    ...
    with open_maybe_gzip(path) as fh:
        for raw_line in fh:
            ...
            if line.startswith("##INFO=<ID="):
                ...
                break
    has_str_annotations = (
        REQUIRED_INFO_FIELDS.issubset(info_ids)
        and REQUIRED_FORMAT_FIELDS.issubset(format_ids)
    )
    ...
    if (
        ...
        if len(fields) > format_col_idx + 2:
            ...
            if gt_idx is not None:
                ...
                # if neither "/" nor "|" found, genotype_separator stays None
